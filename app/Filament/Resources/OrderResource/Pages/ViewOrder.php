    ...
    public function infolist(Infolist $infolist): Infolist
    {
        return $infolist->schema([
            Section::make('Invoice Summary')->schema([
                ...
                TextEntry::make('remaining_amount')->money('USD'),
                TextEntry::make('status')->badge(),
                TextEntry::make('closed_at')->dateTime(),
                TextEntry::make('commission_rate')->formatStateUsing(fn($state): string => $state ? "{$state}%" : '-'),
                TextEntry::make('commission_amount')->money('USD'),
            ])->columns(2),
            Section::make('Notes')->schema([
                TextEntry::make('notes')->placeholder('-')->columnSpanFull(),
                TextEntry::make('deal_notes')->placeholder('-')->columnSpanFull(),
                TextEntry::make('offer_notes')->placeholder('-')->columnSpanFull(),
            ...
            Section::make('Payments')->schema([RepeatableEntry::make('payments')->schema([
                TextEntry::make('payment_date')->date(),
                TextEntry::make('amount')->money('USD'),
                TextEntry::make('payment_method'),
                TextEntry::make('notes')->placeholder('-'),
            ])->columns(4)]),
            Section::make('Old Product Details / تفاصيل المنتجات القديمة')
                ->collapsed()
                ->schema([RepeatableEntry::make('orderItems')->schema([
