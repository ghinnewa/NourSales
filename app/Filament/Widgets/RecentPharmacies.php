    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('owner_name')->label('اسم المالك'),
                Tables\Columns\TextColumn::make('phone'),
                Tables\Columns\TextColumn::make('area'),
                Tables\Columns\TextColumn::make('current_balance')->state(fn (Pharmacy $record) => $record->currentBalance())->money('USD')->label('الرصيد الحالي'),
                Tables\Columns\TextColumn::make('latest_invoice_date')
                    ->label('تاريخ آخر فاتورة')
                    ->state(fn (Pharmacy $record) => optional($record->orders()->latest('invoice_date')->first())->invoice_date)
                    ->date(),
            ])
            ->emptyStateHeading('لا توجد صيدليات بعد.');
    }
}
